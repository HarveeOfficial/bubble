@extends('layouts.app')
@section('title', 'Create Answer Key - OMR Sheet Checker')

@section('content')
<div class="mb-6">
    <a href="{{ route('answer-keys.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to Answer Keys</a>
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Create Answer Key</h1>
    <p class="text-gray-500 mt-1">Define the correct answers for an exam.</p>
</div>

<form action="{{ route('answer-keys.store') }}" method="POST" id="answer-key-form">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="choices_per_item" id="choices_per_item"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm border px-3 py-2">
                    @for($i = 2; $i <= 5; $i++)
                        <option value="{{ $i }}" {{ old('choices_per_item', 4) == $i ? 'selected' : '' }}>{{ $i }} (A{{ $i >= 2 ? '-' . chr(64 + $i) : '' }})</option>
                    @endfor
                </select>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <p class="text-sm text-gray-500 mb-4">Select the correct answer for each question.</p>
    ...
    <div class="flex justify-end space-x-3">
        <a href="{{ route('answer-keys.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
            Cancel
        </a>
    ...
    function renderAnswers() {
        ...
        for (let i = 1; i <= total; i++) {
            ...
            for (let c = 0; c < choices; c++) {
                ...
                const checked = (oldAnswers[i] === label) ? 'checked' : '';
                html += `
