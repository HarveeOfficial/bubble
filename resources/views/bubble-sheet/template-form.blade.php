...
<form action="{{ route('bubble-sheet.generate') }}" method="GET" target="_blank" id="template-form">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <select name="answer_key_id" id="answer_key_id"
                    ...
                    @foreach($answerKeys as $key)
                        <option value="{{ $key->id }}"
                                data-total="{{ $key->total_items }}"
                                data-choices="{{ $key->choices_per_item }}"
                                data-name="{{ $key->name }}"
            ...
            <div>
                ...
                <input type="number" name="total_items" id="total_items" value="{{ old('total_items', 50) }}" min="1" max="200" required
                       class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm border px-3 py-2">
            ...
            <div>
                <label for="columns" class="block text-sm font-medium text-gray-700 mb-1">Number of Columns <span class="text-red-500">*</span></label>
                <select name="columns" id="columns"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm border px-3 py-2">
                    <option value="1" {{ old('columns', 2) == 1 ? 'selected' : '' }}>1 Column</option>
                    <option value="2" {{ old('columns', 2) == 2 ? 'selected' : '' }}>2 Columns</option>
                    <option value="3" {{ old('columns', 2) == 3 ? 'selected' : '' }}>3 Columns</option>
                </select>
            ...
            <div>
                <label for="copies" class="block text-sm font-medium text-gray-700 mb-1">Number of Copies <span class="text-red-500">*</span></label>
                <input type="number" name="copies" id="copies" value="{{ old('copies', 1) }}" min="1" max="10" required
                       class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm border px-3 py-2">
    ...
    <div class="flex justify-end space-x-3">
        <a href="{{ route('dashboard') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
            Cancel
        </a>
        <button type="submit" class="px-6 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition shadow-sm inline-flex items-center">
...
@push('scripts')
<script>
    document.getElementById('answer_key_id').addEventListener('change', function () {
        const selected = this.options[this.selectedIndex];
        if (this.value) {
            document.getElementById('total_items').value = selected.dataset.total;
            document.getElementById('choices_per_item').value = selected.dataset.choices;
            document.getElementById('title').value = selected.dataset.name || '';
            document.getElementById('subject').value = selected.dataset.subject || '';

            // Disable fields when using answer key
            document.getElementById('total_items').disabled = true;
            document.getElementById('choices_per_item').disabled = true;
        } else {
            document.getElementById('total_items').disabled = false;
            document.getElementById('choices_per_item').disabled = false;
        }
    });

    // Re-enable disabled fields on submit so values are sent
